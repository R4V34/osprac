...
int main(int argc, char argv[]){
    ...
    if (stat(directory, &st) == -1) {
        printf(Directory doesn't exists. Try to create newn);
        if(mkdir(directory, 0700)) {
            printf(Can't create dirn);
            exit(-1);
        }
    }
    ...
    while(1){
        ...
        if(fopen(current, r) == NULL) {
            printf(Recursion %dn, count);
            if (closedir(dir) != 0) {
                printf(Error closing dir!n);
                return(-1);
            }
            exit(1);
        }
